...
import com.paymentslab.provider.hostedwebview.HostedCheckoutScreen
 ...
 * instead of a whole [com.paymentslab.provider.hostedwebview.HostedGatewayConfig] list — the mock
 * consent page's "Authorize"/"Cancel" links (see backend `ConnectRoutes`) land on
 * `/mock/connect/{id}/complete` or `/mock/connect/cancelled`, and by the time the WebView observes
 * either, the server-side transition has already happened (or been declined); [StripeConnectOnboarding]
 * just needs to know which one fired.
 */
@Composable
