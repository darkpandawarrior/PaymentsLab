package com.paymentslab.provider.square
 ...
 *
 * The backend's `SquareAdapter` decides real vs mock via `PLAB_SQUARE_TEST_APPLICATION_ID` (from
 * `core:config`, same auto-degrade pattern as `PaystackAdapter`/`PayPalAdapter`) and hands the
 * (non-secret, client-embeddable) Application ID down through [CreatedOrder.providerParams] — the
 * access token that actually charges the resulting nonce never leaves the backend.
 *
 ...
 *
 * **Mock** (Application ID absent): [SimulatedPayment] — no live sandbox credentials were available
 * this session (see `docs/providers/square.md`), so this ships `MOCK_MODE` by default.
 *
