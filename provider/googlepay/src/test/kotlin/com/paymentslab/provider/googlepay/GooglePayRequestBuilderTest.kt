package com.paymentslab.provider.googlepay
 ...
 * this session — this test covers the one part of the integration that's pure logic and genuinely
 * verifiable on the JVM: the request JSON this app sends actually matches Google's documented shape.
 * Robolectric only because `org.json.JSONObject` is a stub on plain JVM unit tests otherwise.
 */
...
class GooglePayRequestBuilderTest {
    ...
    fun `merchant name flows through to merchantInfo`() {
        ...
        assertEquals("PaymentsLab", json.getJSONObject("merchantInfo").getString("merchantName"))
    }
