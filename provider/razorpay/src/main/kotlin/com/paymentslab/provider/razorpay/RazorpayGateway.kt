 ...
 *      [RazorpayCallbackRelay] (see this module's README for the exact code).
...
class RazorpayGateway(
    ...
    override val meta: GatewayMeta = GatewayMeta(
        ...
        blurb = "Razorpay Standard Checkout via the official SDK. Client success is confirmed " +
    ...
    override suspend fun prepare(created: CreatedOrder): PreparedPayment {
        ...
        val razorpayOrderId = p["order_id"]
            ?: throw PaymentPreparationException("Razorpay order missing order_id")

        val params = mapOf(
            "key_id" to keyId,
            "order_id" to razorpayOrderId,
            "amount" to created.order.amount.amountMinor.toString(),
            "currency" to created.order.amount.currency,
        )
        AppLog.d(TAG, "prepared Razorpay order=${created.order.orderId} rzpOrder=$razorpayOrderId")
    ...
    private fun buildOptions(keyId: String, orderId: String, prepared: PreparedPayment): JSONObject =
        JSONObject().apply {
            put("key", keyId)
            put("order_id", orderId)
            put("amount", prepared.amount.amountMinor) // paise
            put("currency", prepared.amount.currency)
            put("name", "PaymentsLab")
            put("description", "Order ${prepared.orderId}")
        }

    /** Map a relayed Razorpay callback into the normalized [PaymentResult]. */
    internal fun mapCallback(callback: RazorpayCallbackResult): PaymentResult = when (callback) {
        is RazorpayCallbackResult.Success -> {
            ...
            AppLog.i(TAG, "Razorpay success paymentId=$paymentId (unverified client claim; server verifies signature)")
        ...
        is RazorpayCallbackResult.Error -> {
            ...
            AppLog.w(TAG, "Razorpay error code=${callback.code} desc=${callback.description}")
     ...
     */
    private fun mapErrorCode(rzpCode: Int): FailureCode = when (rzpCode) {
    ...
    private fun failure(code: FailureCode, message: String, rawReason: String): PaymentResult.Failure =
        PaymentResult.Failure(
            code = code,
