package com.paymentslab.provider.upiintent
...
class UpiIntentGateway : PaymentGateway {
    ...
    override suspend fun pay(
        ...
        return suspendCancellableCoroutine { cont ->
            ...
            try {
                ...
                AppLog.e(TAG, "failed to launch UPI chooser", e)
                finishOnce(
    ...
    internal fun parseUpiResult(activityResult: ActivityResult): PaymentResult {
        ...
        if (response.isNullOrBlank()) {
            AppLog.d(TAG, "UPI returned null/blank response (resultCode=${activityResult.resultCode}) → Cancelled")
        ...
        return when (status) {
            "SUCCESS" -> {
                ...
                AppLog.i(TAG, "UPI Status=SUCCESS (unverified client claim) txnId=$txnId")
                PaymentResult.Success(paymentId = txnId, verification = verification, raw = raw)
            }

            "SUBMITTED" -> {
                AppLog.i(TAG, "UPI Status=SUBMITTED → Pending(UPI_SUBMITTED)")
            ...
            "FAILURE" -> {
                AppLog.w(TAG, "UPI Status=FAILURE responseCode=$responseCode")
                PaymentResult.Failure(
                ...
                AppLog.w(TAG, "UPI unknown Status='$status' → Cancelled")
