package com.paymentslab.provider.omise
 ...
 *
 * The backend's `OmiseAdapter` decides real vs mock via `PLAB_OMISE_TEST_PUBLIC_KEY` /
 * `_SECRET_KEY` (same auto-degrade pattern as `PaystackAdapter`/`PayPalAdapter`/`SquareAdapter`) and
 * hands the (non-secret, client-embeddable) public key down through [CreatedOrder.providerParams].
 *
 ...
 *
 * **Mock** (public key absent): [SimulatedPayment] — no live sandbox credentials were available
 * this session (see `docs/providers/omise.md`), so this ships `MOCK_MODE` by default.
 *
