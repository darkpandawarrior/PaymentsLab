...
class AndroidDeviceIntegrity(
    ...
    override fun inspect(): SecurityReport {
        ...
        if (report.isCompromised) {
            AppLog.w(TAG, "Device compromised: ${signals.joinToString()}")
        } else {
            AppLog.i(TAG, "Device integrity OK (emulator=$emulator)")
        }
        return report
    }
 ...
 */
internal val ROOT_BINARY_PATHS: List<String> = listOf(
    "/system/bin/su",
    "/system/xbin/su",
    "/sbin/su",
    "/su/bin/su",
...
internal val WRITABLE_SYSTEM_DIRS: List<String> = listOf(
    "/system",
    "/system/bin",
    "/system/sbin",
    "/system/xbin",
...
internal fun isRootTag(tags: String?): Boolean =
...
internal fun isEmulatorBuild(
    ...
    if (fp.startsWith("generic") || fp.startsWith("unknown") || fp.contains("emulator") ||
        fp.contains("sdk_gphone") || fp.contains("vbox")
    ) {
        return true
    }

    val hw = hardware.lowercase()
    if (hw in setOf("goldfish", "ranchu", "vbox86", "ttvm_x86") || hw.contains("goldfish") ||
        hw.contains("ranchu")
    ) {
        return true
    }
    ...
    if (mdl.contains("sdk_gphone") || mdl.contains("emulator") || mdl.contains("android sdk built for") ||
    ...
    if (prod.contains("sdk_gphone") || prod.contains("sdk_google") || prod == "google_sdk" ||
        prod.contains("emulator") || prod.contains("vbox86") || prod.startsWith("sdk")
    ) {
        return true
    }
