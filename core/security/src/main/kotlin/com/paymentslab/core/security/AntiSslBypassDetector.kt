...
/**
 * Best-effort detector for SSL/TLS pinning-bypass attempts, ported from Dice's production
 * `AntiSslBypassDetector`. This is a **complement to** — never a replacement for — real certificate
...
object AntiSslBypassDetector {
    ...
    fun detect(): List<String> {
        ...
        if (signals.isNotEmpty()) {
            AppLog.w(TAG, "SSL-bypass indicators: ${signals.joinToString()}")
        }
        return signals
    }
