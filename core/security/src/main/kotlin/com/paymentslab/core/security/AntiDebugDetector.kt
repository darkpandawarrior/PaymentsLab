...
/**
 * Runtime anti-debugging detector, ported from Dice's production `AntiDebugDetector`.
 *
 ...
 * There is no timing trick here (Dice's timing heuristic was noisy and false-positive-prone on cold
...
object AntiDebugDetector {
    ...
    fun detect(context: Context): List<String> {
        ...
        if (signals.isNotEmpty()) {
            AppLog.w(TAG, "Debug indicators: ${signals.joinToString()}")
        }
        return signals
    }
