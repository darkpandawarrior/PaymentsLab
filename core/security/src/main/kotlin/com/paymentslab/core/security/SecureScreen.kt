...
fun SecureScreen(
    ...
    DisposableEffect(enabled) {
        ...
        if (enabled && activity != null) {
            ...
            AppLog.w(TAG, "SecureScreen enabled but no host Activity found; screenshots NOT blocked")
        }

        onDispose {
            if (enabled && activity != null) {
                activity.window.clearFlags(WindowManager.LayoutParams.FLAG_SECURE)
                AppLog.d(TAG, "FLAG_SECURE cleared from ${activity.javaClass.simpleName}")
            }
        }
    }
