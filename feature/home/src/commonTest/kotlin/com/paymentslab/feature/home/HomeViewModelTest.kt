...
class HomeViewModelTest {
    ...
    @Test
    fun gateway_count_reflects_the_full_registry_not_just_sandbox_ready() = runTest {
        val vm = HomeViewModel(mixedStatusRegistry(), FakePendingPaymentJournal())
        assertEquals(5, vm.uiState.value.gatewayCount)
    }

    @Test
    fun success_rate_is_the_percentage_of_terminal_successes_among_resolved_payments() = runTest {
        val journal =
    ...
    @Test
    fun success_rate_is_zero_with_no_resolved_payments_not_a_divide_by_zero_crash() = runTest {
        val vm = HomeViewModel(mixedStatusRegistry(), FakePendingPaymentJournal())
        vm.uiState.test {
    ...
    @Test
    fun recent_activity_is_the_three_newest_payments() = runTest {
        val journal =
            FakePendingPaymentJournal(
                listOf(
                    payment("o1", "razorpay", PaymentStatus.SUCCESS, createdAt = 100),
